...
def sev_of(c):
    ...
    return (
        s if hashable(s) and s in SEVERITIES else "major"
    )  # unknown or absent -> major
...
def clean_conflicts(act):
    ...
    return [
        {**c, "kind": kind_of(c)}
        for c in dicts(act.get("conflicts"))
...
def accepted(exp):
    ...
    return {
        x if hashable(x) else json.dumps(x, sort_keys=True, default=str) for x in vs
    }
...
def load(path, kind):
    ...
    try:
        ...
        ) as fh:  # tolerate a BOM from Windows editors
    ...
    if out:  # cheap shape check: catches swapped arguments
        looks_actual = sum(
            1 for o in out if {"confidence", "route", "unknowns"} & set(o)
        )
        looks_key = sum(
            1
            for o in out
            if {"forbidden_kinds", "alternates", "clean", "negative_control"} & set(o)
            or any(
                isinstance(c, dict) and "min_severity" in c
                for c in as_list(o.get("conflicts"))
            )
        )
        if kind == "expected" and looks_actual > len(out) / 2 and not looks_key:
            print(
                f"score.py: WARNING {path} looks like agent output, not a key — "
                f"did you swap the arguments?",
                file=sys.stderr,
            )
        if kind == "actual" and looks_key > len(out) / 2 and not looks_actual:
            print(
                f"score.py: WARNING {path} looks like a key, not agent output — "
                f"did you swap the arguments?",
                file=sys.stderr,
            )
    ...
    if dupes:
        ...
        print(
            f"score.py: {path}: duplicate request_id {dupes} — keeping the first of each",
            file=sys.stderr,
        )
        first, deduped = set(), []
        for o in out:
        ...
        for o in out:
            ...
            for fld in ("conflicts", "alternates", "forbidden_kinds", "site_map"):
                if fld in o and not isinstance(o[fld], list):
                    die(
                        f"{path} {rid}: {fld} must be a list, got {type(o[fld]).__name__}"
                    )
            ...
            for variant in [o] + [{**o, **a} for a in o.get("alternates", [])]:
                ...
                for sm in variant.get("site_map", []):
                    if not (isinstance(sm, (list, tuple)) and len(sm) >= 2):
                        die(
                            f"{path} {rid}: site_map entries must be [requested, resolved] pairs"
                        )
                ...
                if dup:
                    die(
                        f"{path} {rid}: conflict kind listed twice ({dup}); "
...
def validate_key(expected):
    ...
    for e in expected:
        ...
        if any(
            set(v.get("forbidden_kinds", [])) < set(e.get("forbidden_kinds", []))
            for v in variants
        ):
        ...
        for v in variants:
            ...
            if "blocking" in floors and not (vacc & REFUSALS):
                problems.append(
                    f"{rid}: a variant demands a blocking conflict but accepts "
                    f"only {sorted(vacc)} — unreachable at 2/2"
                )
            elif ("major" in floors or "blocking" in floors) and vacc <= {"feasible"}:
                problems.append(
                    f"{rid}: a variant demands a major conflict but accepts only "
    ...
    if n and not any(e.get("forbidden_kinds") for e in expected):
        notes.append(
            "no case forbids any kind — a shotgun answer cannot be caught by points; "
            "read shotgun_index and clean_case_precision"
        )
    if n and n_conf < max(1, n // 3):
        notes.append(
            f"only {n_conf}/{n} cases expect any conflict — expected for the "
    ...
    if n:
        ...
        if sb >= 0.75:
            problems.append(
                f"a constant feasible/no-conflict stub scores {sb} against this "
                f"key — it does not measure enough to quote"
            )
        elif sb >= 0.34:
            notes.append(
                f"stub_baseline {sb}: a constant answer already earns that much; "
                f"read the sub-metrics, not the headline"
            )
    return {
        "cases": n,
        "with_conflicts": n_conf,
        "notes": notes,
        "clean": sum(1 for e in expected if e.get("clean")),
        "negative_controls": sum(1 for e in expected if e.get("negative_control")),
        "kinds": dict(
            Counter(c["kind"] for e in expected for c in e.get("conflicts", []))
        ),
        "problems": problems,
    }
...
def _score_one(exp, act, boiler=frozenset(), allowance=None):
    ...
    need_res = {
        c["kind"]
        for c in exp_conf
        if c.get("resolution_required", RANK[c.get("min_severity", "major")] < 2)
    }
    ...
    resolved_ok = all(
        any(real_resolution(x) for x in act_conf if x["kind"] == k) for k in need_res
    )

    conflicts_ok = (
        (n_match == n_exp) and resolved_ok and not hit_forbidden and discriminate_ok
    )
...
def score_case(exp, act, boiler=frozenset()):
    ...
    allowance = (
        min(len({c["kind"] for c in dicts(v.get("conflicts"))}) for v in cands) + 2
    )
    ...
    out = max(
        (_score_one(e, act, boiler, allowance) for e in cands),
...
def score(expected, actual):
    ...
    for exp in expected:
        ...
        per_case.append(
            {"request_id": rid, "points": p, "reason": why, "unexpected_kinds": unexp}
        )
        ...
        if exp.get("clean"):
            ...
            clean_ok += (
                0
                if act is None
                else int(not any(sev_of(x) in ("blocking", "major") for x in conf))
            )
        if exp.get("negative_control"):
            neg_t += 1
            ...
            neg_ok += (
                0
                if act is None
                else int(hashable(av) and av in (accepted(exp) & REFUSALS) and kinds_ok)
            )
        if exp.get("resolved_sequence") is not None:
            ...
            seq_ok += (
                ...
                if act is None
                    ...
                    and norm(act.get("resolved_sequence"))
                    == norm(exp["resolved_sequence"])
                )
            )
        if exp.get("site_map"):  # a key that lists no pairs asserts nothing here
            ...
            got = (
                {
                    (norm(m.get("requested")), norm(m.get("resolved")))
                    for m in dicts(act.get("site_map"))
                }
                if act
                else set()
            )
            ...
            map_ok += int(act is not None and want <= got and len(got) <= len(want) + 3)
    ...
    return {
        ...
        "accuracy_by_confidence": {
            k: round(conf_hits[k] / v, 4) for k, v in conf_tot.items()
        },
        ...
        "unexpected_outputs": sorted(
            (set(by_id) - {e["request_id"] for e in expected}),
...
def brier(forecasts, results):
    if not isinstance(results, dict):
        die(
            f"results.json: expected an object of request_id -> 0/1/2, got "
            f"{type(results).__name__} (generate it with `score.py --results`)"
        )
    if not isinstance(forecasts, dict):
        die(
            f"forecasts.json: expected an object of request_id -> probability, got "
            f"{type(forecasts).__name__}"
        )
    for k, v in results.items():
        if isinstance(v, bool) or not isinstance(v, int) or v not in (0, 1, 2):
            die(
                f"results.json: {k} = {v!r}; expected integer 0/1/2 "
    ...
    out = {
        ...
        "outcome_base_rate": round(ybar, 4),
        "imputed_missing_forecasts": imputed,
        "coerced_forecasts": coerced,
...
def agreement(expected_ids, runs):
    ...
    for rid in expected_ids:
        ...
        sets = [
            {c["kind"] for c in clean_conflicts(r[rid])} if rid in r else {"<missing>"}
            for r in runs
        ]
    ...
    return {
        "per_case": per,
        "stability": round(
            sum(v["verdict_agreement"] * v["kind_stability"] for v in per.values())
            / len(per),
            4,
        )
        if per
...
def audit(actual, workbook):
    ...
    res = {
        "resolved": 0,
        "inference_refs_checked": 0,
        "unsourced_blocks": 0,
        "uncited_inference_blocks": 0,
        "dead_refs": [],
        "header_row_refs": [],
        "empty_row_refs": [],
        "wrong_workbook_refs": [],
    ...
    def check(ref):
        ...
        elif row in HEADER_ROWS.get(sheet, set()) or any(
            text.upper().lstrip().startswith(h) for h in HEAD
        ):
...
def applicability(requests, actual):
    ...
    MARKERS = {
        ...
        "diacid": "lipidation",
        "fatty ac": "lipidation",
        "grubbs": "hydrocarbon_stapling",
        "metathes": "hydrocarbon_stapling",
        "staple": "hydrocarbon_stapling",
        "pegylat": "pegylation",
        "mpeg": "pegylation",
        "glycos": "glycosylation",
        "sugar": "glycosylation",
        "disulfid": "disulfide",
        "acm": "disulfide",
        "iodine": "disulfide",
        "lactam": "cyclization",
        "macrocyc": "cyclization",
        "cycliz": "cyclization",
        "cyclis": "cyclization",
        "retro-inverso": "retro_inverso",
        "n-methyl": "n_methylation",
    }
    by_req = {
        r["request_id"]: r
        for r in requests
        if isinstance(r, dict)
        and hashable(r.get("request_id"))
    ...
    for obj in actual:
        ...
        fams = {
            m.get("family")
            for m in dicts(req.get("modifications"))
            if hashable(m.get("family"))
        }
        fams |= {
            w
            for f in as_list(req.get("parent_features"))
            for w in (
                ["cyclization"]
                if "lactam" in str(f).lower()
                else ["disulfide"]
                if "disulfid" in str(f).lower()
                else []
            )
        }
...
def main():
    ...
    try:
        if a and a[0] == "--brier":
            if len(a) < 3:
                die("--brier needs forecasts.json results.json")
            if len(a) > 3:
                die("--brier takes exactly two files")
            print(json.dumps(brier(jload(a[1]), jload(a[2])), indent=2))
        elif a and a[0] == "--agreement":
            if len(a) < 4:
                die("--agreement needs expected.jsonl and at least two runs")
            ids = [e["request_id"] for e in load(a[1], "expected")]
            runs = [
                {
                    o["request_id"]: o
                    for o in load(p, "actual")
                    if o.get("request_id") is not None
                }
                for p in a[2:]
            ]
            ...
            if len(a) < 3:
                die("--applies needs requests.jsonl actual.jsonl")
            if len(a) > 3:
                die("--applies takes exactly two files")
            ...
            if len(a) < 3:
                die("--audit needs actual.jsonl and one or more workbooks")
            out = audit(load(a[1], "actual"), a[2:])
            print(json.dumps(out, indent=2))
            total = (
                out["resolved"]
                + out["uncited_inference_blocks"]
                + out["unsourced_blocks"]
            )
            sys.exit(
                1
                if (
                    out["dead_refs"]
                    or out["wrong_workbook_refs"]
                    or out["header_row_refs"]
                    or out["empty_row_refs"]
                    or (
                        total
                        and (out["uncited_inference_blocks"] + out["unsourced_blocks"])
                        / total
                        > 0.5
                    )
                )
            ...
            if len(a) < 2:
                die("--validate-key needs expected.jsonl")
            if len(a) > 2:
                die("--validate-key takes exactly one file")
            out = validate_key(load(a[1], "expected"))
            print(json.dumps(out, indent=2))
            sys.exit(1 if out["problems"] else 0)
        elif a and a[0] == "--validate":
            if len(a) < 3:
                die("--validate needs actual.jsonl schema.json")
            if len(a) > 3:
                die("--validate takes exactly two files")
            try:
                import jsonschema
            except ImportError:
            ...
            with open(
                a[1], encoding="utf-8-sig"
            ) as fh:  # RAW: load() repairs, validate must not
            ...
            if len(a) < 3:
                die("--results needs expected.jsonl actual.jsonl")
            if len(a) > 3:
                die("--results takes exactly two files")
            s = score(load(a[1], "expected"), load(a[2], "actual"))
            print(
                json.dumps(
                    {c["request_id"]: c["points"] for c in s["per_case"]}, indent=2
                )
            )
        elif len(a) >= 2:
            as_json = "--json" in a
            ...
            if as_json:
                ...
                for c in pc:
                    extra = (
                        f"  +{c['unexpected_kinds']}" if c["unexpected_kinds"] else ""
                    )
